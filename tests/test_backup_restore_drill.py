...
def _postgres_server():
    ...
    if external:
        ...
        # normally SHARED: the rest of the suite writes to it. The drill below destroys
        # its own data with a hardcoded `DROP DATABASE "agentcms"`, so the drill has to
        # own a database actually named `agentcms` -- exactly as the Docker and initdb
        # strategies do. Reusing the shared database instead means the drill seeds into
        # suite-wide state (CI: `assert 5 == 3`, `UniqueViolation ix_sites_slug`) and
        # "destroys" a database it never used, which makes the destroy/restore half of
        # the drill vacuous.
        from sqlalchemy.engine import make_url

        admin_url = make_url(external).set(database="postgres").render_as_string(hide_password=False)
        url = make_url(external).set(database="agentcms").render_as_string(hide_password=False)

        _run_ddl(admin_url, 'DROP DATABASE IF EXISTS "agentcms" WITH (FORCE)')
        _run_ddl(admin_url, 'CREATE DATABASE "agentcms"')
        ...
        try:
            yield {"url": url, "admin_url": admin_url, "cleanup": lambda: None, "mode": "external"}
        finally:
            dispose_engine()
            _run_ddl(admin_url, 'DROP DATABASE IF EXISTS "agentcms" WITH (FORCE)')
        return
    ...
    if docker_available():
        ...
        url = f"postgresql+psycopg://postgres:postgres@127.0.0.1:{port}/agentcms"
        ...
        yield {"url": url, "admin_url": admin_url, "cleanup": cleanup, "mode": "docker"}
        return
    ...
    url = f"postgresql+psycopg://postgres@127.0.0.1:{port}/agentcms"
    ...
    if created.returncode != 0:
        ...
        with engine.connect() as conn:
            conn.execute(text('CREATE DATABASE "agentcms"'))
        engine.dispose()
    ...
    yield {"url": url, "admin_url": admin_url, "cleanup": cleanup, "mode": f"initdb ({version})"}
...
class TestFullBackupRestoreDrill:
    ...
    def test_seed_backup_destroy_restore_verify(self, postgres_server):
        ...
        with engine.connect() as conn:
            conn.execute(
                text("SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'agentcms'")
            )
            conn.execute(text('DROP DATABASE IF EXISTS "agentcms"'))
        engine.dispose()
...
class TestDrillIsolation:
    ...
    * the drill's hardcoded ``DROP DATABASE "agentcms"`` dropped a database it never
    ...
    def test_drill_owns_its_database_when_a_server_is_external(self, postgres_server):
        ...
        assert drill_db == "agentcms", (
            f"drill runs against database {drill_db!r}; it destroys and recreates "
            "'agentcms', so that is the only name that can work"
        )
        assert make_url(postgres_server["url"]).database != shared_db, "drill shared the suite database"
