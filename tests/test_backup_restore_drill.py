...
def postgres_server():
    """Start a throwaway PostgreSQL server (Docker or local initdb) for the drill."""
    ...
    if external:
        ...
        yield {"url": external, "admin_url": admin_url, "cleanup": lambda: None, "mode": "external"}
        return
...
def seed_database(database_url: str) -> dict:
    ...
    with session_scope() as session:
        ...
        post_ids = []
        for i, (slug, title, body, status) in enumerate([
...
class TestFullBackupRestoreDrill:
    ...
    def test_seed_backup_destroy_restore_verify(self, postgres_server):
        ...
        # --- STEP 2: Seed data ---
        seed_data = seed_database(database_url)
        ...
        engine = create_engine(admin_url, isolation_level="AUTOCOMMIT")
        with engine.connect() as conn:
            conn.execute(text("SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'agentcms'"))
        ...
        proc = _run([
            sys.executable, "-m", "scripts.restore",
            "--dump", str(backup_file),
...
class TestUpgradeRollbackDrill:
    ...
    def test_upgrade_rollback_no_data_loss(self, postgres_server):
        ...
        # Seed initial data
        seed_data_v1 = seed_database(database_url)
        original_counts = count_core_tables(database_url)
        original_hashes = compute_content_hashes(database_url)
        ...
        with session_scope() as session:
            site = session.execute(
                text("SELECT id FROM sites WHERE slug = 'drill-blog'")
            ).scalar_one()
            actor = session.execute(
                text("SELECT id FROM actors WHERE label = 'Drill Actor'")
            ).scalar_one()
