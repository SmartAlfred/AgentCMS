...
```
POST   /v1/sites              create a site            (scope: sites:write)
GET    /v1/sites              list sites               (scope: sites:read)
GET    /v1/sites/{slug}       read one site            (scope: sites:read)
```

A fresh instance has no site: `POST /v1/sites` is the first call a self-hoster makes
(`scripts/deploy_smoke.sh` does exactly this). The slug is lowercase and
hyphen-separated; a duplicate slug is a `409 site-slug-conflict` and never an
overwrite, so a re-run can safely fall back to `GET /v1/sites/{slug}`.
